...
- Все сообщения коммитов в Git - на русском
...
### Что такое Git для нетехнических людей:
Git - это как машина времени для твоего кода. Она запоминает каждое изменение, которое ты делаешь, чтобы можно было вернуться к любой версии.

### ВАЖНО: Все сообщения коммитов на русском!

Когда ты сохраняешь изменения в Git (это называется "коммит"), ты ОБЯЗАН писать описание на русском языке.

**Примеры ПРАВИЛЬНЫХ сообщений коммитов:**
```
git commit -m "Исправлена ошибка с отображением изображений"
git commit -m "Добавлена новая секция с товарами"
git commit -m "Обновлены стили кнопок по дизайну из Figma"
git commit -m "Изменен цвет фона на кремовый"
```

**Примеры НЕПРАВИЛЬНЫХ сообщений (не делай так!):**
```
git commit -m "Fixed bug"  ❌ (на английском)
git commit -m "Update styles" ❌ (на английском)
git commit -m "asdfgh" ❌ (бессмысленный текст)
```

### Простые правила для сообщений коммитов:
1. Пиши что ты сделал простыми словами
2. Начинай с глагола в прошедшем времени (Добавлен, Исправлен, Обновлен)
3. Будь конкретным (не "Изменения", а "Изменен размер заголовка")
4. Используй русский язык ВСЕГДА
...
- НЕ пиши коммиты на английском языке
